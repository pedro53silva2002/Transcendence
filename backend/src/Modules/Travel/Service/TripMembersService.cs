...
public sealed class TripMembersService(TripMembersModel tripMembersModel, ICachingService cacheService)
{
	...
	public async Task<TripMembersDto> UpdateAsync(int userId, int id, UpdateTripMembersDto dto, CancellationToken ct = default)
	{
		...
		await cacheService.RemoveAsync($"user:me:{tripmember.UserId}", ct);
	...
	{
		var member = await tripMembersModel.GetById(DeletingId, ct)
