...
export class PlanATripComponent implements OnInit {
  ...
  planATripForm = this.formBuilder.nonNullable.group({
    trip: this.formBuilder.nonNullable.group({
      ...
      country: [
        null as CountryDto | null,
        [Validators.required, PlanATripComponent.countryValidator],
      ],
      ...
      if (!this.isEditMode) {
        if (formValue.trip.country) {
          const dto: CreateTripDto = {
            ...
            budget: Number(formValue.trip.budget),
          ...
          this.tripService.create(dto).subscribe({
            next: (response) => {
              if (response.data?.id) this.currentTripId.set(response.data.id);
              if (response.data !== undefined) this.tripStateService.setTrip(response.data);
...
export class PlanATripComponent implements OnInit {
	...
	constructor() {
		effect(() => {
			...
			const tripForm = this.tripForm();
			if (tripForm) {
				try {
					tripForm.updateFormFields(
						currentTrip?.city ?? [],
	...
	ngOnInit(): void {
		...
		if (this.isEditMode) {
			let currentTrip = this.tripStateService.trip();
