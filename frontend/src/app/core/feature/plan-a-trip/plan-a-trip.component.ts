...
export class PlanATripComponent implements OnInit {
            ...
            if (currentTrip) {
                this.planATripForm.patchValue({
                    trip: {
                        ...
                        // CORREÇÃO: Convertemos a string da BD para um objeto Date real.
                        // O PlainDateAdapter vai ler isto e mostrar "DD/MM/YYYY" sem fusos horários.
            ...
            if (!this.isEditMode) {
                if (formValue.trip.country) {
                    const dto: CreateTripDto = {
                        ...
                        // Enviamos estritamente "YYYY-MM-DD"
    ...
    static countryValidator(control: AbstractControl): ValidationErrors | null {
        ...
        if (typeof value === 'string') return { countryNotSelected: true };
        if (typeof value === 'object' && !value.id) return { countryNotSelected: true };
        return null;
    }
